...
bool GameSession::take_activate()
{
	...
	if (unit) {
		...
		if (unit->take_item(displayed_items->at(disp_items_num))) {
			...
			My_vect::swap(displayed_items->at(disp_items_num), displayed_items->back());
...
bool GameSession::game_end_check() const
{
	bool status = false;
	for (size_t i = 0; i < player.size(); i++)
...
void GameSession::draw() const
{
	...
	switch (current_mode) {
        ...
        case mode::game:
			{
				for (size_t i = 0; i < unit_vision.size(); i++) {
					...
					cout << std::setw((con_len - unit_vision.size()) / 2);

					for (size_t j = 0; j < unit_vision[i].size(); j++)
						if ((current_mode == mode::attack) && 
							(i == (attack_cursor.y - current_unit->pos().y + current_unit->vision())) && 
							(j == (attack_cursor.x - current_unit->pos().x + current_unit->vision())))
							cout << '*';
						else
		...
		default:
            std::cout << "Draw function error: unknown game mode.\n";
            break;
    }
